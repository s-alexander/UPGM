...
paycheck::CConfig::CConfig ()
{

}

int paycheck::CConfig::ReadFile(const char *filename)
{
	Clear ();
...
int paycheck::CConfig::ParseText(const char *text)
{
    if (text==NULL)
        return -1;
    return ParseBuffer(text, strlen(text));
}

const char *paycheck::CConfig::GetValue (const char *aParam)
{
	return GetValue (aParam, NULL);
}

int paycheck::CConfig::SetValue (const char *aParam, const char *new_val)
{
	return SetValue (aParam, NULL, new_val);
}

const char *paycheck::CConfig::GetValue (const char *aParam, const char *Section)
{
	unsigned int idx = 0;
...
int paycheck::CConfig::SetValue (const char *aParam, const char *Section, const char *new_value)
{
	if (new_value==NULL)
		return -1;
...
void paycheck::CConfig::Clear ()
{
	std::vector <CConfigElement *>::iterator it;
...
int paycheck::CConfig::ParseBuffer (const char *buffer, int size)
{
bool new_line=true;
...
if (buffer==NULL) return 1;
	...
	const char *paramstart = buffer;
	unsigned int paramsize = 0;
	const char *valuestart = NULL;
	const char *sectionstart=buffer;
